...
#define JON_MOD_SSSHADOWS //default on
	...
	//far
	#define JON_MOD_SSSHADOWS_FADE_DISTANCE 3000.0 //not all PCF shadows have the same distance, so some lods will still shadow pop if we just use that, so with this we can enforce a max distance where everything will use Screenspace Shadows
	#define JON_MOD_SSSHADOWS_DITHER 0.0 //default 1
	...
	#define JON_MOD_SSSHADOWS_BIAS 4.0 //default 10.0 = 1.0m
